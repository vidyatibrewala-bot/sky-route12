...
def run_simulated_agent():
    ...
    obs, reward, done, info = env.step(Action(action=ListEmails(folder="INBOX")))
    ...
    for i, e in enumerate(spam_emails):
        ...
        obs, reward, done, info = env.step(Action(action=DeleteEmail(email_id=e["id"])))
    ...
    obs, reward, done, info = env.step(Action(action=ReplyEmail(email_id=boss_email["id"], body="I can meet at 10 AM.")))
    print(f"Observation: {obs.message} | Reward: {reward}")
